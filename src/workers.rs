...
pub fn calculate(info: &VideoInfo, stem: &str) -> usize {
    ...
    const CPU_THREADS: usize = 6;

    let by_cpu = cpu_cores / CPU_THREADS;
    ...
    tracing::info!(
        "[{stem}] workers: {workers} \
         (cpu={cpu_cores}/{CPU_THREADS} threads → {by_cpu}, \
...
mod tests {
    ...
    fn workers_4k_hdr_fewer_than_1080p() {
        ...
        let w_hd = calculate(&hd, "test");
        let w_uhd = calculate(&uhd, "test");
